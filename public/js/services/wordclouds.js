    ...
    function createCommonWordArray(answers) {
        ...
        answers.forEach(function(answer) {
            ...
            tokens.forEach(function(token) {
                ...
                if (termsForThisAnswer.indexOf(sanitizedToken) === -1 && stopList.indexOf(sanitizedToken) === -1) // only count each unique word in an answer
        ...
        for (var key in dictionary) {
            if (dictionary.hasOwnProperty(key)) {
                formattedArray.push({
                    text: key,
                    size: (dictionary[key]/numTotalTerms) * 300 // size is weighted proportionally to the frequency of the term
                });
            }
        }
